...
2. El siguiente circuito implementa un filtro activo:
    ...
    1. Muestre que la frecuencia con la que se obtiene la ganancia maxima es $\omega \to +\infty$. (0.5pt)
    1. Determine la(s) frecuencia(s) de corte en Hz, asumiendo que $R=1\ k\Omega$ y $C=0.1\ \mu F$. (1pt)
    1. Considere que $A = (1+\frac{R_a}{R_b})$. Al aumentar el valor de A, eventualmente el filtro se vuelve inestable. Determine el valor de A en que esto ocurre y con qué frecuencia oscila $v_o$, en Hz. (0.7pt)
