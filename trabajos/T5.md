...
<img src="https://julianodb.github.io/electronic_circuits_diagrams/T5_comp_pulse_gen.png" width="450">

Figura 2: Comparador en la entrada del generador de pulsos

Cuando $V_+ > V_-$ la salida del comparador está cortada y, por lo tanto, la resistencia $R_{50}$ conecta la entrada del generador de pulsos a +5V. Anteriormente supusimos que la entrada de dicho circuito se conectaba directamente a +5V cuando elegimos los valores de sus componentes ($C_3$, $R_{12}$, $R_{13}$, $R_{14}$ y $R_{15}$). Para evitar que $R_{50}$ afecte el funcionamiento del generador de pulsos proyectado, es necesario eligir un valor pequeño lo suficiente para que se pueda considerar aproximadamente un corto-circuito desde el punto de vista de la entrada del generador de pulso.

3. Determinen la impedancia de entrada $Z_{in}$ del circuito generador de pulsos a muy alta frecuencia ($\omega \to +\infty$). Para realizar el cálculo, pueden considerar que tanto $D_1$ como la unión base-emisor de $Q_7$ conducen. (1pt)

4. Elijan un valor para $R_{50}$ de tal forma que $R_{50} \approx \frac{Z_{in}}{10}$. Asegúrense que, con el valor elegido, la corriente que entra en la salida del comparador cuando $V_+ < V_-$ es menor que la corriente de corto-circuito del LM393LVDR. (1pt)
...
<img src="https://julianodb.github.io/electronic_circuits_diagrams/phototransistor.png" width="300">
...
<img src="https://julianodb.github.io/electronic_circuits_diagrams/T5_ir_detect.png" width="300">
...
Será necesario hacer un ajuste fino cuando se fabriquen las placas para seleccionar el valor de $R_{49}$, pero en este trabajo eligiremos un valor preliminar para esta resistencia.

5. Consulten la hoja de datos del emisor infrarrojo (VSMB10940) e identifiquen la intensidad radiante cuando la corriente es 20 mA. Luego, consulten la hoja de datos del fototransistor (PT12-21C-TR8) e identifiquen cuanto es la corriente de colector cuando detecta luz ($I_{C(ON)}$) y cuanta intensidad luminosa produce dicha corriente. 

6. Estimen cuanta intensidad luminosa recibe el fototransistor si está a 15 cm del emisor infrarrojo. Luego, asumiendo que la corriente de colector es linealmente proporcional a la intensidad luminosa, cuanta corriente de colector se generaría en dicho caso.

7. Finalmente, elijan un valor de $R_{29}$, disponible en el anexo, que baje $V_-$ a menos de 2.5 V cuando uno de los fototransistores genera la corriente estimada en el item anterior. 
...
9. Elijan un valor de capacitancia disponible en el anexo para $C_{7}$ de tal forma que cuando se enciende la fuente (+5V), el circuito demora menos de 5 segundos para llegar a 99% de su valor en estado estacionario. Elijan el mayor valor de $C_{7}$ que cumple con el requerimiento. (1pt)
...
Como la impedancia de salida del circuito es muy alta, no se pueden conectar circuitos que consumen mucha corriente a $V_{bias\ weak}$ , pues se cambiaria el valor de este. Una forma de entenderlo es considerar que al conectar una resistencia entre $V_{bias\ weak}$ y tierra, ésta apareceria en paralelo a $R_{22}$, disminuyendo su valor efectivo y, por lo tanto disminuyendo el valor de $V_{bias\ weak}$ . Para mejorar la capacidad de la tierra virtual de entregar o consumir corrientes se utilizará un buffer, el que se muestra en la figura a continuación.

<img src="https://julianodb.github.io/electronic_circuits_diagrams/buffer_vbias.png" width="300">

Figura 4: Buffer

El buffer es un circuito que utiliza un amplificador operacional para producir un voltaje de salida idéntico al de entrada. La utilidad del buffer está en tener una impedancia de entrada muy alta y una impedancia de salida muy baja, lo que ayuda a desacoplar la señal de entrada de la salida. Es decir, consumir muy poca corriente de la señal de entrada, al mismo tiempo en que se entrega mucho más corriente en en circuito de salida. $V_{bias}$ es la tierra virtual que efectivamente se conectará a los circuitos que la requieren.
